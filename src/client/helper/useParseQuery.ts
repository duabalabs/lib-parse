...
interface State<T extends Parse.Object<Parse.Attributes>>
...
type Action<T extends Parse.Object<Parse.Attributes>> =
  ...
  | UnloadResultAction<T>
  | ResetAction<T>
  | ReturnType<typeof setIsLive | typeof setIsSyncing | typeof fail>;
          ...
          index = results.findIndex(
            (result) =>
              compareParseObjects(
                ...
                action.payload.order as string[]
              ) >= 0
          );
        }
...
export interface UseParseQueryResult<T extends Parse.Object<Parse.Attributes>>
  extends ResultState<T> {
  reload: () => void;
}
...
export const useParseQuery = <T extends Parse.Object<Parse.Attributes>>(
  ...
  const { results: initialResults, count: initialCount } = initialLoad || {};

  const stateRef = useRef<State<T>>(getInitialState(1, initialLoad));

  const [
    { queryId, isLoading, isLive, isSyncing, results, count, error },
    dispatch,
  ] = useReducer<Reducer<T>>((state, action) => {
  ...
  const localDatastoreQuery: Parse.Query<T> | undefined = useMemo(() => {
    if (enableLocalDatastore) {
      ...
      const memoedQuery = Parse.Query.fromJSON(
        queryJSON.className,
        queryJSON.query
      ) as Parse.Query<T>;
  ...
  const findFromLocalDatastore = useCallback(async () => {
    let findResult;
    try {
  ...
  const subscribeLiveQuery = useCallback(() => {
    ...
    const loadAndPinResult = async (result: T) => {
      dispatch(
        loadResult(
          queryId,
          result as T,
          (parseServerQuery as any)._order,
          (parseServerQuery as any)._limit
        )
      );
    ...
    const subscribe = async () => {
      ...
      liveQuerySubscription.on("create", (result) =>
        loadAndPinResult(result as T)
      );
      liveQuerySubscription.on("update", (result) =>
        loadAndPinResult(result as T)
      );
      liveQuerySubscription.on("enter", (result) =>
        loadAndPinResult(result as T)
      );
      liveQuerySubscription.on("leave", (result) =>
        unloadAndUnPinResult(result as T)
      );
      liveQuerySubscription.on("delete", (result) =>
        unloadAndUnPinResult(result as T)
      );
    };
